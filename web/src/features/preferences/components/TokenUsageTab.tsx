...
export function TokenUsageTab() {
  ...
  return (
    <div>
        ...
        {usage ? (
          <div className="space-y-3">
            ...
            <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-muted">
              <div
                className="h-full bg-ring/40 transition-[width] duration-200 ease-out"
            ...
            <div className="flex flex-col gap-2 text-caption font-mono tabular-nums sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-ring/40" />
      ...
      <div className="mt-6">
        <h4 className="mb-3 text-body-sm-bold text-foreground">
        ...
        <div className="overflow-hidden rounded-lg border border-border">
          {/* Header */}
          <div
            className="hidden gap-3 bg-muted px-4 py-3 label-mono text-muted-foreground md:grid"
                  ...
                  >
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 font-mono text-caption tabular-nums text-muted-foreground sm:grid-cols-4 md:contents md:mt-0">
                      <span className="flex justify-between gap-2 md:block md:text-right">
                        <span className="text-text-subtle md:hidden">{t("profile.inputTokens")}</span>
                        {formatTokenCount(item.input_tokens)}
                      </span>
                      <span className="flex justify-between gap-2 md:block md:text-right">
                        <span className="text-text-subtle md:hidden">{t("profile.outputTokens")}</span>
                        {formatTokenCount(item.output_tokens)}
                      </span>
                      <span className="flex justify-between gap-2 md:block md:text-right">
                        <span className="text-text-subtle md:hidden">{t("preferences.usage.modelResponses")}</span>
                        {item.request_count}
                      </span>
                      <span className="flex justify-between gap-2 font-sans md:block md:text-right">
                        <span className="text-text-subtle md:hidden">{t("preferences.usage.lastActive")}</span>
